/*----------------------------------------------------------------------------
 * StirMark -- GIF Support
 *
 * (c) 1997-1998 Fabien A.P. Petitcolas
 *
 * This code is based on ppmtogif on the PBMPLUS distributionof December 1991.
 ...
 *
 * $Header: /StirMark/ppmtogif.c 3     1/12/98 21:15 Fapp2 $
 *----------------------------------------------------------------------------
 */
...
static colorhash_table ppm_colorhisttocolorhash(colorhist_vector chv, int colors);
    ...
    for (i = 0; i < colors; ++i)
    {
        ...
        for (chl = cht[hash]; chl != (colorhist_list) 0; chl = chl->next)
            if (PPM_EQUAL(chl->ch.color, color))
                ERROR( "same color found twice - %d %d %d", PPM_GETR(color),
...
/*----------------------------------------------------------------------------
 */
void ImageWriteGIF(IMAGE I, FILE *fout)
{
    int colors, i, BitsPerPixel;
    int interlace = 1; /* Interlaced GIF */
    int Red[MAXCOLORS], Green[MAXCOLORS], Blue[MAXCOLORS];
    pixval maxval = I.max;
    colorhist_vector chv;
    
    if (I.depth == 1)
        ERROR("GIF conversion not supported with greyscale images.");

    pixels = (pixel **)&I.img;
    pI = &I;
    
    /* Figure out the colormap. */
    MESSAGE("computing colormap...");
    chv = ppm_computecolorhist(pixels, I.size.x, I.size.y, MAXCOLORS, &colors);
    if (chv == (colorhist_vector) 0)
    ...
    /* All set, let's do it. */
    GIFEncode(fout, I.size.x, I.size.y, interlace, 0, BitsPerPixel,
...
static int GetPixel(int x, int y)
{
    ...
    color = ppm_lookupcolor(cht, (pixel *)&(pI->img[pI->depth * (y * pI->size.x + x)]));
    return color;
}
